...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\gamechain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\gamechain
    // Mac and Unix: ~/.gamechain
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "gamechainCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "gamechain";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".gamechain-cold";        
    }
    return pathRet / ".gamechain";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"gamechain",".conf") == 0)
    {
...
{
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"gamechain",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc_gamechainParams::SetProtocolGlobals()
{
...
int mc_gamechainParams::SetGlobals()
{
...
int mc_gamechainParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string gamechainServerAddress()
{
