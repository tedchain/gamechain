...
/* MCHN START */
static Checkpoints::MapCheckpoints mapCheckpointsgamechain =
        boost::assign::map_list_of
        ( -1, uint256("0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"))
        ;
static const Checkpoints::CCheckpointData datagamechain = {
        &mapCheckpointsgamechain,
        0,
        0,
        0
    };
...
class CgamechainParams : public CMainParams {
public:
    CgamechainParams() {    };
    
    void SetFixedParams(const char *NetworkName)
    {
        networkID = CBaseChainParams::gamechain;
...
//        convertSeed6(vFixedSeeds, pnSeed6_test, ARRAYLEN(pnSeed6_test));
    ...
    void SetgamechainParam(const char*param_name,int64_t value)
    {
    ...
    void SetgamechainRuntimeParams()
    {
...
//        assert(hashGenesisBlock == uint256("0x0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"));
    ...
    const Checkpoints::CCheckpointData& Checkpoints() const 
    {
        return datagamechain;
    }
};
static CgamechainParams gamechainParams;


bool SelectgamechainParams(const char *NetworkName)
{
    SelectgamechainBaseParams(NetworkName,(int)mc_gState->m_NetworkParams->GetInt64Param("defaultrpcport"));

    gamechainParams.SetFixedParams(NetworkName);
    
    gamechainParams.Initialize();
    
    pCurrentParams = &gamechainParams;
...
void SetgamechainParams()
{
    gamechainParams.SetgamechainParams();
}

void SetgamechainParam(const char*param_name,int64_t value)
{
    gamechainParams.SetgamechainParam(param_name,value);
}


void SetgamechainRuntimeParams()
{
    gamechainParams.SetgamechainRuntimeParams();
}

bool InitializegamechainParams()
{
    
    gamechainParams.Initialize();
    
    gamechainParams.SetGenesis();
