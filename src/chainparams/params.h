...
#ifndef GAMECHAINPARAMS_H
#define	GAMECHAINPARAMS_H
...
typedef struct mc_OnegamechainParam
{    
...
} mc_OnegamechainParam;

typedef struct mc_gamechainParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_OnegamechainParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolgamechain;
    int m_ProtocolVersion;
    ...
    int Clone(const char *name,mc_gamechainParams *source);
    ...
    const mc_OnegamechainParam *FindParam(const char *param);
    ...
    int IsParamUpgradeValueInRange(const mc_OnegamechainParam *param,int version,int64_t value);
