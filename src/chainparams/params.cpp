...
int mc_OnegamechainParam::IsRelevant(int version)
{
    int ret=1;
...
void mc_gamechainParams::Destroy()
{
    if(m_lpData)
    {
        mc_Delete(m_lpData);
...
int64_t mc_gamechainParams::GetInt64Param(const char *param)
{
    int size;
...
double mc_gamechainParams::Int64ToDecimal(int64_t value)
{
    if(value < 0)
    {
...
int64_t mc_gamechainParams::DecimalToInt64(double value)
{
    return (int64_t)(value*MC_PRM_DECIMAL_GRANULARITY+mc_gState->m_NetworkParams->ParamAccuracy());    
}

int mc_gamechainParams::GetParamFromScript(char* script,int64_t *value,int *size)
{
    char *ptr;    
    ptr=script;
    ptr+=strlen(ptr)+1;
...
double mc_gamechainParams::GetDoubleParam(const char *param)
{
...
void* mc_gamechainParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc_gamechainParams::IsParamUpgradeValueInRange(const mc_OnegamechainParam *param,int version,int64_t value)
{
...
int mc_gamechainParams::CanBeUpgradedByVersion(const char *param,int version,int size)
{
    if(m_lpIndex == NULL)
    {
...
int mc_gamechainParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc_gamechainParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc_gamechainParams::Init()
{
    ...
    m_Count=sizeof(gamechainParamArray)/sizeof(mc_OnegamechainParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((gamechainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(gamechainParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(gamechainParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_OnegamechainParam*)mc_New(sizeof(gamechainParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,gamechainParamArray,sizeof(gamechainParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc_gamechainParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_OnegamechainParam *param;
    char *ptrData;
    int num_sets;
...
double mc_gamechainParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


const mc_OnegamechainParam *mc_gamechainParams::FindParam(const char* param)
{
    int i;
    for(i=0;i<m_Count;i++)
    {
        if(strcmp((gamechainParamArray+i)->m_Name,param) == 0)
        {
            return gamechainParamArray+i;
        }        
    }    
    return NULL;
}

int mc_gamechainParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc_gamechainParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc_gamechainParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc_gamechainParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((gamechainParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((gamechainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (gamechainParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (gamechainParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((gamechainParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((gamechainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((gamechainParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc_gamechainParams::Set(const char *name,const char *source,int source_size)
{
...
int mc_gamechainParams::Clone(const char* name, mc_gamechainParams* source)
{
    int err;
...
int mc_gamechainParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc_gamechainParams::Validate()
{
    int i,size,offset;
    int isGenerated;
    int isMinimal;
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"gamechain") == 0)
            {
...
int mc_gamechainParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== gamechain configuration file ====\n\n");
    fprintf(fileHan,"# Created by gamechain-util \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"gamechaind %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun gamechain-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"gamechaind %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"gamechaind %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                ...
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence gamechain network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"gamechain-util clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running gamechaind for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"gamechain-util clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by gamechain-util.\n");
                            fprintf(fileHan,"# They SHOULD ONLY BE EDITED IF YOU KNOW WHAT YOU ARE DOING. \n");
                            break;
                        case MC_PRM_CALCULATED:
                            fprintf(fileHan,"# The following parameters were generated by gamechaind.\n");
...
int mc_gamechainParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc_gamechainParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc_gamechainParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc_gamechainParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc_gamechainParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc_gamechainParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc_gamechainParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc_gamechainParams::ProtocolVersion()
{
...
int mc_gamechainParams::IsProtocolgamechain()
{
    return m_IsProtocolgamechain;
}
...
int mc_Features::VerifySizeOfOpDropElements()                                   // This test is still in the code to keep protocol!-gamechain untouched
{
    
    int ret=0;        
    int protocol=mc_gState->m_NetworkParams->ProtocolVersion();
    
    if(mc_gState->m_NetworkParams->IsProtocolgamechain() == 0)
    {
        return 1;
    }
