...
bool gamechainNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !gamechainNode_CanConnect(pnode);
    }
...
bool gamechainNode_AcceptData(CNode *pnode)
{
    ...
    return !gamechainNode_DisconnectRemote(pnode);
}
...
bool VerifygamechainVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifygamechainVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!gamechainNode_CanConnect(pfrom))
            {
