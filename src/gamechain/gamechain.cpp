...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling gamechaind. Exiting...\n\n",mapArgs["-datadir"].c_str());
        return false;        
    }
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  gamechaind <blockchain-name> [options]                     " + _("Start gamechain Core Daemon") + "\n";
...
//                delete mc_gState;
    ...
    if(err)
    {
        fprintf(stderr,"ERROR: Couldn't read configuration file for blockchain %s. Please try upgrading gamechain. Exiting...\n",mc_gState->m_Params->NetworkName());
        delete mc_gState;                
        return false;
    }
    ...
    if(err)
    {
        ...
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Please restart gamechaind with reindex=1.\n",mc_gState->m_Params->NetworkName());                        
        }
        else
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Probably gamechaind for this blockchain is already running. Exiting...\n",mc_gState->m_Params->NetworkName());
        }
        delete mc_gState;                
        return false;
    }
    ...
    if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
    {
        ...
        if(mc_gState->IsSupported(protocol_version) == 0) 
        {
            ...
            {
                fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in gamechain %s\n\n",
            ...
            {
                fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by gamechain running newer protocol version (%d)\n\n",
                        mc_gState->m_Params->NetworkName(),protocol_version);                        
                fprintf(stderr,"Please upgrade gamechain\n\n");
                delete mc_gState;                
                return false;
            }
        }
    ...
    switch(mc_gState->m_NetworkParams->m_Status)
    {
        ...
        case MC_PRM_STATUS_MINIMAL:
            if(mc_gState->GetSeedNode() == NULL)
            {
                ...
                fprintf(stderr,"  gamechain-util create %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  gamechain-util clone <old-blockchain-name> %s\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"\nAnd rerun gamechaind %s\n\n\n",mc_gState->m_Params->NetworkName());                        
                fprintf(stderr,"If you want to connect to existing blockchain please specify seed node:\n\n");
                fprintf(stderr,"  gamechaind %s@<seed-node-ip>\n",mc_gState->m_Params->NetworkName());
                fprintf(stderr,"  gamechaind %s@<seed-node-ip>:<seed-node-port>\n\n\n",mc_gState->m_Params->NetworkName());
                delete mc_gState;                
                return false;
            }
            break;
        ...
        case MC_PRM_STATUS_INVALID:
            ...
            fprintf(stderr,"  gamechain-util clone %s <new-blockchain-name>\n",mc_gState->m_Params->NetworkName());
            delete mc_gState;                
            return false;
...
/*        
        ...
        if (fDaemon)
        {
            fprintf(stdout, "gamechain server starting\n");
